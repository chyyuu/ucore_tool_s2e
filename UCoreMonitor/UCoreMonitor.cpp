...
void UCoreMonitor::initialize(){
  ...
  m_UCoreVersion = s2e()->getConfig()->getString(getConfigKey() + ".OSVersion");
...
void UCoreMonitor::slotCall(S2EExecutionState *state, uint64_t pc){
  uint64_t callAddr;
  callAddr = cast<klee::ConstantExpr>
    (State->readCpuRegister(CPU_OFFSET(regs[R_EIP]),
                            8 * sizeof(target_ulong)))->getZExtValue();
  // for debug
